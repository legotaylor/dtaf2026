...
void main() {
	...
	vec3 realCellSize = CellSize;
	realCellSize.y *= 5; // You can adjust the cloud thickness with this
	vec3 realCloudOffset = CloudOffset;
	//realCloudOffset.y += 50; Uncomment this to adjust the cloud height
	vec3 pos = (faceVertex * realCellSize) + (vec3(cellX, 0, cellZ) * realCellSize) + realCloudOffset;
	//pos.y -= (fog_spherical_distance(pos) / 5); World curvature type thing, best used while the cloud layer is much higher up
	gl_Position = ProjMat * ModelViewMat * vec4(pos, 1.0);

	vertexDistance = fog_spherical_distance(pos);
	vertexColor = (useTopColor ? faceColors[1] : faceColors[direction]) * CloudColor;
	vertexColor = (vertexColor * 0.5) + (CloudColor * 0.5);

	PlayerPos = pos;

	Pos = vec3(faceVertex * realCellSize) + (vec3(cellX, 0, cellZ) * realCellSize);
}
